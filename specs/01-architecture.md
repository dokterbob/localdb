...
- **Single-writer rule (LanceDB gate):** an advisory file lock (`<data_dir>/.write.lock`)
  ...
  for the duration of an actual LanceDB write — not during embedding, metadata ops, or reads.
...
- **Runtime-state concurrency:** the runtime-state registry (`runtime-state.db`) is backed by
  ...
  write lock within 2 s surfaces as `runtime_state_locked` (exit 4). Metadata operations
  (store/source add, list) are never blocked by the LanceDB write gate.
