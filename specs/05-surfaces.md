...
| `job list`                                                 | List every job on a daemon's job queue (issue #218-followups Fix A), regardless of state or store; daemon-only like `job cancel` — `-s` is rejected, exit 2 (§2.2). Table columns: id, store, state, error_code, created_at; `--json` emits the raw `IndexJob[]` array `GET /v1/jobs` returns                                                                                                                                                                                                    | n/a — exit 5 (`daemon_unreachable`) without a running daemon                                              | `GET /v1/jobs`                                                                                                                                                                                                                                                                                          |
...
is now that same scaffolding step run explicitly, with repair semantics on top: it unconditionally
ensures the `default` store exists, even against a config file another command already scaffolded or
a user hand-wrote.
...
- **`DELETE /jobs/{id}`** (issue #218): requests cancellation of a queued or running job. `202` +
  ...
  **Deliberate design decision:** cancellation does not add a fifth `IndexJobState` — it reuses
  ...
  is not a guarantee the job was actually interrupted** (issue #218-followups Fix D): `409` is
