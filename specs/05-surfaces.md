...
Every command/tool first probes the daemon socket/lock ([01-architecture.md](01-architecture.md)
§3): daemon present → thin client over its HTTP API; absent → embedded mode (open store in-process).
...
| `serve`                                                    | Run the daemon (HTTP API, watching, refresh, socket/lock); serves every store regardless, so `-s` is rejected, exit 2 (§2.2)                                                                                                                                                                                                                                                                                                                                                                   | becomes the daemon                                                                                        | error `daemon_running`                                                                                                                                                                                                                                                                                |
