...
| `added_at`                  | When this resource _version_ was first indexed — our clock (`now()` at ingestion), never a source value. Because `id` is content-addressed (`blake3(uri ‖ content_hash)`, §3), a content change mints a new resource ID and therefore a fresh `added_at`; a policy-only re-index (chunking/embedding config change, content unchanged) preserves the existing `added_at` — the write path carries the stored value forward when replacing the same resource ID (lands with the migration-v7 write-path change, §9). |
...
Like `index_updated_at` itself, this document-level exposure lands with the migration-v7 wave (§9);
until then `DocumentInfo` carries only `fetched_at` (= `added_at`) alongside the metadata blob.
