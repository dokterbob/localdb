...
| `last_checked_at`           | Our clock: when we last **successfully contacted the origin** for this resource's URI — a `200` or a `304` — and left the store consistent for it. Advanced by every path that reaches that outcome: the feed entry loop, `url` sources, single-document feed mode, and (as before) the liveness sweep — see [04-search-pipeline.md](04-search-pipeline.md) §1 "What a probe writes" and "Recheck gate" for the per-path write and the one deliberate exception (the sweep also advances it on `Blocked`/transport-error, for its own oldest-first rotation, disclosed there). **Never** bumps `index_updated_at`. **Never advanced** on an error, `Blocked` (no origin contact happened), a confirmed-`Gone` delete (the row is gone; a discovery-mode entry that falls back to embedded content instead retains its row via `on_resource_fallback`, which likewise never advances this column), an empty extraction, or a recheck-gate skip — those write paths deliberately leave `doc_index` stale so the next run retries, and stamping this clock would let the recheck gate suppress exactly that retry ([04-search-pipeline.md](04-search-pipeline.md) §1 "Recheck gate"). **Deliberately not one of the four date axes** and not `DateAxis`-filterable — see the note under the date-axes table in §2. A **separate column, never a reuse of `index_updated_at`**: `index_updated_at` normatively means "when we last wrote this resource's stored state" and is exposed publicly as `DocumentInfo.index_updated_at` through `localdb document get`, `GET /v1/documents/{id}`, and MCP `get_document`/`list_documents`; a successful check that leaves content and metadata unchanged writes nothing there, so bumping it would report a document as re-written when it was only checked. |
...
`core::store::DateAxis` has four variants; `resources.last_checked_at` — our clock for the last
successful origin contact on a resource's URI (§2) — is neither of those, and is not
`DateAxis`-filterable. It is a separate column for exactly the conflation hazard this table exists
to teach against. It sits adjacent in kind to `index_updated_at` (both are our clock) and differs on
the one point that matters: a successful check that leaves content and metadata unchanged writes
nothing to `index_updated_at`, so folding the two together would report a document as re-written
when it was only checked. Keeping it off `DateAxis` is that same judgement one layer up — it is a
**throttle on origin contact**, not a claim about the document and not a version axis, and it is
`NULL` for every resource no successful check has reached: every file-sourced resource (the `file`
ingestor never touches this column at all) and every `url`/feed resource this store has not yet
fetched or `304`'d at least once. Under the `NULL` rule in "Filtering" below, a filter on it would
silently exclude nearly every document in a mixed store. Revisiting it needs two things that are not
true today: semantics that have stopped moving, and a demonstrated operator need that the `--json`
index summary's `feed_entries_liveness_checked`/`docs_recheck_deferred` counters do not already
meet.
...
> re-fetching and re-indexing unchanged entries, is mostly avoided by the recheck gate
> ([04-search-pipeline.md](04-search-pipeline.md) §1 "Recheck gate"), which skips the HTTP round
> trip for an already-known, in-floor entry outright; for whatever still falls through to a fetch,
> the entry's own resource-level validators keep that a cheap `304` rather than a full re-download.
> The 304 short-circuit on the feed document itself is a deliberate, accepted trade-off, not a
> mitigated one. When the feed document returns 304, the entry loop does not run at all, so zero
> entry callbacks fire that run — drift on an already-indexed entry's own _page_ goes unnoticed for
> as long as the feed XML itself is unchanged. **In discovery mode that run reports no document at
> all (normative):** the document counters partition `docs_seen`, and in discovery mode the feed
...
> (RFC 9110 §8.8.1). `--refetch` ([05-surfaces.md](05-surfaces.md)) is the cache-busting escape
> hatch: it bypasses the recheck gate's floor check for the run **and** suppresses that run's
> feed-document validators — the same effect as a `feed_inputs_digest` mismatch above — so it is not
> a no-op for a feed whose document itself would otherwise 304. Bypassing only the gate would still
> dead-end at the document 304 before the entry loop ever ran; both halves are needed together. What
> a 304'd run's empty seen-set means for the liveness sweep is settled in one place, next to the
> guard's normative definition: [04-search-pipeline.md](04-search-pipeline.md) §1 "Aged-out feed
...
[04-search-pipeline.md](04-search-pipeline.md) §1), `docs_recheck_deferred` (feed entries
gate-skipped without an HTTP request, a sub-count of `docs_skipped` —
[04-search-pipeline.md](04-search-pipeline.md) §1 "Recheck gate"), `chunks_written`,
`unsupported_format_count`, `error_count`, and `sources_count` (size of the job's resolved scope
before processing, distinguishing "nothing to index" from "sources existed but nothing needed
indexing"). Both embedded and daemon-submitted jobs run through the same async engine
(`server::job_exec::run_job`, driven by a `JobQueue`) — the CLI's embedded mode spins up its own
in-process, single-job `JobQueue` per invocation rather than running synchronously outside the job
model; the daemon's `JobQueue` is long-lived and serves every job for the process's lifetime, one at
a time per store (a per-store in-flight guard rejects a second concurrent submission with
`index_in_progress`) ([05-surfaces.md](05-surfaces.md) §3).
