...
**Closure note (issue #79).** A residual same-run window remained after the above: the
replace delete (`delete_by_document`) still ran in its own transaction, separate from the
