...
- **HTTP daemon is experimental**: reads (`/v1/search`, `/v1/documents/{id}`, `/v1/status`) DO see CLI-indexed data — the daemon opens the same unified `localdb.db` as the CLI, not an in-memory store, and there is no separate write-lock (SQLite WAL + `busy_timeout=5000` serialise concurrent writers). But `POST /v1/jobs` (ingestion) is a no-op — it reports success with zero stats and never runs ingestion (issue #187) — and `localdb index` silently proxies to it whenever a daemon is running. **Stop the daemon before running `localdb index`.** See `specs/05-surfaces.md §3` and `docs/architecture.md#known-gaps`.
