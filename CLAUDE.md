...
- **HTTP daemon is experimental**: `POST /v1/jobs` (ingestion) is currently a no-op. Reads (`/v1/search`, `/v1/documents/{id}`, `/v1/status`) DO see CLI-indexed data — the daemon opens the same unified `localdb.db` as the CLI, not an in-memory store. CLI commands keep working while the daemon runs: SQLite WAL + `busy_timeout=5000` serialise concurrent writers instead of failing outright (there is no separate write-lock). See `specs/05-surfaces.md §3` and `docs/architecture.md#known-gaps`.
